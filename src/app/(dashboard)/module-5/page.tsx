// src/app/(dashboard)/module-5/page.tsx
...
export default async function Module5Page() {
  ...
  if (module4) {
    const module4Progress = await ModuleService.getModuleProgress(user.id, module4.id)
    if (module4Progress < 100) {
      return (
        <div className="container mx-auto p-6 max-w-5xl">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
            <h2 className="text-xl font-semibold mb-2">Module Locked</h2>
            <p className="text-gray-600 mb-4">
              Please complete Module 4 to unlock Module 5
            </p>
            <Link href="/module-4">
              <Button>Go to Module 4</Button>
            </Link>
          </div>
        </div>
      )
